...
class KYParseHtml(ParserBase):
    def __init__(self, input_file_name):
        super().__init__()
        self.class_regex = {'ul': '^CHAPTER', 'head2': '^CHAPTER', 'title': '^(TITLE)|^(CONSTITUTION OF KENTUCKY)', 'sec_head': r'^([^\s]+[^\D]+)',
        ...
        if re.search('constitution', self.html_file_name):
            for main_tag in self.soup.find_all("p"):
                    if main_tag.text.strip() == "CONSTITUTION OF KENTUCKY":
                        ...
                        continue
                    elif re.match(r'Preamble|SCHEDULE|ORDINANCE',main_tag.text.strip()) and main_tag.get("class") == [self.class_regex['ul']]:
                       continue
    ...
    #         #     "h4").text.strip() == 'NOTES TO DECISIONS'or "NOTES TO UNPUBLISHED DECISIONS" and note_tag.find_next().name != "span":
    #             # note_tag.name = "li"
    #
    #         # parent
    ...
    #
    #         # child
    #         if re.match(r'^(\d+\.\s*—\s*[a-zA-Z]+)|^(\d+\.\d+)', note_tag.text.strip()) :
    #             note_tag.name = "li"
    ...
    #                     "h4").text.strip() == 'NOTES TO DECISIONS'or "NOTES TO UNPUBLISHED DECISIONS" and p_tag.find_next().name != "span":
    #
    ...
    def add_citation(self):
        title_dict = {"I": ['1', '2', '3'], "II": ['5', '6', '6A', '7', '7A', '7B', '8'],
                      ...
                      'X': ['116', '117', '117A', '118', '118A','118B','119', '120', '121', '121A', '122', '123', '124',
                            '125', '126', '127', '128'],
                ...
                for match in [x[0] for x in re.findall(r'((Ky.\s*(L. Rptr.\s*)*\d+)|'
                    ...
                    if re.search(r'^(\d+\D*\.\d+)', tag.text.strip()):
                        ...
                        if re.search(r'(U.S.C.\s*secs*\.\s*\d+(\([a-zA-Z]\))*(\(\d+\))*)', match.strip()):
                            cite = re.search(r'((?P<USC>U.S.C.\s*secs*\.\s*\d+)(\((?P<USCSEC>[a-zA-Z])\))*(\((?P<USCOL>\d+)\))*)', match.strip())
                            if cite.group("USC") and cite.group("USCSEC") and cite.group("USCOL"):
                                tag_text = f'#{cite.group("USC")}{cite.group("USCSEC")}{cite.group("USCOL")}'
                                tag_id = re.sub("\s",'',tag_text)
                            elif cite.group("USC") and cite.group("USCSEC"):
                                tag_text = f'#{cite.group("USC")}{cite.group("USCSEC")}'
                                tag_id = re.sub("\s", '', tag_text)
                            else:
                                tag_text = f'#{cite.group("USC")}'
                                tag_id = re.sub("\s", '', tag_text)
                            ...
                            text = re.sub(fr'\s{re.escape(match)}',
                                          f'<cite class="US_code"><a href="{tag_id}" target="{target}">{match}</a></cite>',
                                          inside_text, re.I)
                            tag.append(text)

                        #(Ky.\s?(App\.)?\s?LEXIS\s?\d+)
                        if re.search(r'(Ky.\s?(App\.)?\s?LEXIS\s?\d+)', match.strip()):
                            cite = re.search(r'(?P<APP>Ky.\s?(App\.)?\s?LEXIS\s?\d+)', match.strip()).group("APP")
                            tag_id = re.sub("\s",'',cite)

                            tag.clear()
                            target = "_blank"

                            text = re.sub(fr'\s{re.escape(match)}',
                                          f'<cite class="ky_app_code"><a href="#{tag_id}" target="{target}">{match}</a></cite>',
                                          inside_text, re.I)
                            tag.append(text)
                        ...
                        if re.search(r'(Ky.\s*(L. Rptr.\s*)*\d+)|(OAG \d+-\d+)', match.strip()):
                            if re.search(r'(?P<APP>(Ky.\s*(L. Rptr.\s*)*\d+))', match.strip()):
                                cite = re.search(r'(?P<APP>(Ky.\s*(L. Rptr.\s*)*\d+))', match.strip()).group("APP")
                                tag_id = re.sub("\s",'',cite)

                                if re.match(r'(?P<APP>(Ky.\s*(L. Rptr.\s*)\d+))', match.strip()):
                                    cite_class = "ky_rptr_code"
                                else:
                                    cite_class = "ky_code"

                            else:
                                cite = re.search(r'(?P<APP>OAG \d+-\d+)', match.strip()).group("APP")
                                tag_id = re.sub("\s", '', cite)
                                cite_class = "OAG"
                            ...
                            text = re.sub(fr'\s{re.escape(match)}',
                                          f'<cite class="{cite_class}"><a href="#{tag_id}" target="{target}">{match}</a></cite>',
                                          inside_text, re.I)
                            tag.append(text)
    ...
    def wrap_with_ordered_tag(self):
        ...
        pattern = re.compile(r'^(\d+\.(\s*[a-z]\.)*)|^(\(\d+\)|^\(\s*[a-z][a-z]?\s*\))|^([a-z]\.)|^([A-Z]{0,3}\.)|^([a-z]{0,3}\.)')
        Num_bracket_pattern = re.compile(r'^\(\d+\)')
        alpha_pattern = re.compile(r'^\(\s*[a-z][a-z]?\s*\)')
        # alp_pattern = re.compile(r'\(\D+\)')
        num_pattern = re.compile(r'^\d+\.')
        # num_pattern1 = re.compile(r'^1\.')
        numAlpha_pattern = re.compile(r'^\s*\(\d+\)\s*\(\D\)\s*')
        alphanum_pattern = re.compile(r'^\(\D+\)\s(\d)+')
        ...
        # I.
            if re.match(r'^([I,V,X]{0,3}\.\s*[a-zA-Z]+)', current_tag):
                ...
                if re.match(r'^([I,V,X]{0,3}\.)', current_tag):
                    ...
                    tag_id = re.search(r'^(?P<id>[I,V,X]{0,3})', current_tag).group('id')
                    prev_header_id = tag.find_previous("h3").get("id")
                    tag["id"] = f"{prev_header_id}ol1{tag_id}"
         ...
         # # (1)
            if re.match(Num_bracket_pattern, current_tag):
                ol_num = tag
                if re.search(r'^(\(1\))', current_tag):
                    if re.match(r'^([I,V,X]{0,3}\.)',tag.find_previous().text.strip()):
                ...
                if header_tag.get("class") == [self.class_regex["title"]]:
                    ...
                    if re.search(r'^§(§)*|^(ARTICLE)|^(AMENDMENTS)', header_tag.text.strip()):
                        ...
                        if re.search(r'^§(§)*', header_tag.text.strip()):
                            chap_num = re.search(r'^(§(§)*\s*(?P<chap>\d+[a-zA-Z]*).?) ', header_tag.text.strip()).group(
                                "chap").zfill(2)
                    ...
                    if re.search(r'^Section', header_tag.text.strip()):
                        ...
                        if header_tag.find_previous("h3") and re.match(r'AMENDMENT',header_tag.find_previous("h2").text.strip()):
                            prev_id = header_tag.find_previous("h3").get("id")
                            header_tag.name = "h4"
                        else:
                            prev_id = header_tag.find_previous("h2").get("id")
                        cur_id = re.search(r'^^(Section\s?(?P<sec>\d+).)',header_tag.text.strip()).group("sec").zfill(2)
                ...
                if header_tag.get("class") == [self.class_regex["title"]]:
                    ...
                    if re.search("^(CHAPTER)|^(Chapter)", header_tag.text):
                        ...
                        if re.match("^([A-Z]\.)",header_tag.text):
                            subpart_nums = re.search(r'^((?P<chapter_id>[A-Z])\.)',header_tag.text.strip()).group("chapter_id").zfill(2)
                            header_tag["id"] = f"{prev_id}sub{subpart_nums}"

                        if re.match(r'^(Subpart)\s(?P<chapter_id>\w+)',header_tag.text):
                            subpart_nums = re.search(r'^(Subpart)\s(?P<chapter_id>\w+)',header_tag.text.strip()).group("chapter_id").zfill(2)
                            header_tag["id"] = f"{prev_id}sub{subpart_nums}"
                    ...
                    if re.match(r'^(\d+[a-z]?\.\d+[a-z]?-\d+\.)', header_tag.text.strip()):
                        chap_num = re.search(r'^([^.]+)', header_tag.text.strip()).group().zfill(2)
                        sec_num = re.search(r'^(\d+[a-z]?\.\d+[a-z]?-\d+)', header_tag.text.strip()).group().zfill(2)
                        header_tag["id"] = f"t{self.title_id}c{chap_num}s{sec_num}"
                    ...
                    if re.search("^(CHAPTER)|^(Chapter)", header_tag.text):
                        ...
                        # print(header_tag)
                        art_nums = re.search(r'^(Article|SUBCHAPTER(S)*)\s(?P<chapter_id>\w+)', header_tag.text.strip()).group(
                            'chapter_id')
                        if header_tag.find_previous_sibling().name != "li":
                            anav = 0
                        anav = anav+ 1
        ...
        for list_item in self.soup.find_all("li"):
            if re.search('constitution', self.html_file_name):
                if re.match(r'^(§(§)*)', list_item.text.strip()):
                    chap_num = re.search(r'^(§(§)*\s*(?P<chap>\d+[a-zA-Z]*).?) ', list_item.text.strip()).group("chap").zfill(2)
                    nav_list = []
                    nav_link = self.soup.new_tag('a')
                    nav_link.append(list_item.text)
                ...
                if re.match(r'^(CHAPTER)|^(Chapter)', list_item.text.strip()):
                    ...
                    if re.match(r'^(\d+\.\d+\.)', list_item.text.strip()):
                        ...
                        if re.match(r'(\d+[a-zA-Z]*\.\d+-\d+\.)', list_item.text.strip()):
                            chap_num = re.search(r'^([^.]+)', list_item.text.strip()).group().zfill(2)
                            sub_num = re.search(r'(\d+[a-zA-Z]*\.(?P<sub>\d+)-\d+\.)', list_item.text.strip()).group("sub").zfill(2)
                        ...
                        art_nums = re.search(r'^(Article|SUBCHAPTER(S)*)\s(?P<chapter_id>\w+)', list_item.text.strip()).group(
                            'chapter_id')
                        new_link = self.soup.new_tag('a')
                        new_link.string = list_item.text
    ...
    def create_ul_tag_to_notes_to_decision2(self):
        ...
        for note_tag in self.soup.find_all(class_=tag_class):
           ...
           # # parent
                    if re.match(r'^(\d+\.\s*“?[a-zA-Z]+)|^(\d+\.\s*“?\d*)', note_tag.text.strip()) and note_tag.name == "li":
            ...
            for p_tag in self.soup.find_all(class_=notetodecison_nav_class):
                if re.match(r'^(\d+\.)', p_tag.text.strip()):
                    if p_tag.find_next().name != "span":
                        ...
                        if re.search(r'^(\d+.\s*[a-zA-Z]+)',p_tag.get_text()):
                            prev_head_tag = p_tag.find_previous("h4").get("id")
                            ...
                            if re.match(r'^1.',p_tag.text.strip()):
                                nav_link_list = []
                                count = 1
                        ...
                        sub_sec = re.sub(r'[\s—.]', '', p_tag.get_text()).lower()
                        sub_sec_id = p_tag.find_previous("h5").get("id")
                        innr_nav_link1 = self.soup.new_tag('a')
                        innr_nav_link1.string = p_tag.text
                        innr_nav_link1["href"] = f"#{sub_sec_id}-{sub_sec}"
    ...
    def start_parse(self):
        ...
        self.css_file()
        if re.search('constitution', self.html_file_name):
            self.class_regex = {'ul': '^(§ )|^(ARTICLE)', 'head2': '^(§ )|^(ARTICLE)', 'title': '^(CONSTITUTION OF KENTUCKY)|^(THE CONSTITUTION OF THE UNITED STATES OF AMERICA)',
                                'sec_head': r'^([^\s]+[^\D]+)|^(Section)',
                                'junk': '^(Text)', 'ol': r'^(\(1\))', 'head4': '^(NOTES TO DECISIONS)|^(Compiler’s Notes.)'}
